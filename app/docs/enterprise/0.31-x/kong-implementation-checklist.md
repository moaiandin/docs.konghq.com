...
- Allow HTTP traffic to [proxy_listen](/docs/latest/configuration/#proxy_listen). By default  8000. 
- Allow HTTPS traffic to [proxy_listen_ssl](/docs/latest/configuration/#proxy_listen_ssl). By default 8443.
- Allow HTTP traffic to [admin_listen](/docs/latest/configuration/#admin_listen). By default 8001.
- Allow HTTPS traffic to [admin_listen_ssl](/docs/latest/configuration/#admin_listen_ssl). By default 8444.
...
- The upstream APIs behind Kong will be available on [proxy_listen](/docs/latest/configuration/#proxy_listen) and [proxy_listen_ssl](/docs/latest/configuration/#proxy_listen_ssl). Configure these ports accordingly to the access level you wish to grant to the upstream APIs.
- Protect [admin_listen](/docs/latest/configuration/#admin_listen), and only allow trusted sources that can access the Admin API.
...
Note: During an upgrade, the cluster that is processing requests while disconnected from the datastore will not be able to persist Vitals data.

There are plans in the roadmap to improve the upgrade process.
